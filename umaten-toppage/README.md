# Umaten トップページ WordPressプラグイン

グルメポータルサイト「ウマ店」のトップページコンテンツを表示するWordPressプラグインです。

## バージョン 1.3.0 の変更点（スマホ表示最適化版）

### 修正内容

スマホで「エリアから選ぶ」セクションが表示されない問題を完全に修正しました。

#### 主な修正点：

1. **エリアコンテンツの表示制御を強化**
   - `.meshimap-wrapper`プレフィックスを追加してセレクタの詳細度を向上
   - `display: none !important`と`display: block !important`で確実に制御
   - 外部CSSファイル（toppage.css）での制御を追加

2. **480px以下のメディアクエリを大幅強化**
   - 都道府県カードを1列表示に強制変更
   - `grid-template-columns: 1fr !important`で確実に表示
   - カードの高さ、幅、displayプロパティすべてに`!important`を追加
   - エリアタブのレイアウトを最適化

3. **CSSファイル構造の改善**
   - HTMLインラインスタイルと外部CSSファイル（toppage.css）の整合性を確保
   - WordPressテーマとの競合を完全に回避

### 技術的な詳細

#### 修正されたファイル：
- `umaten-toppage-plugin.php`: バージョン番号を1.3.0に更新
- `assets/css/toppage.css`: エリア表示制御とスマホ対応を強化
- `README.md`: ドキュメントを更新
...
### 1.3.0 (2024-11-14)
- **重要**: スマホで「エリアから選ぶ」セクションが表示されない問題を完全修正
- 外部CSSファイル（toppage.css）にエリア表示制御を追加
- 480px以下のメディアクエリを大幅強化
- `.meshimap-wrapper`プレフィックスでセレクタの詳細度を向上
- すべての重要なスタイルに`!important`を追加してWordPressテーマとの競合を完全回避
- プラグイン情報を更新（名称、説明文）
